...
export default function HistoryLaporanPage() {
  ...
  return (
    <>
      <Navbar />
      <div className="min-h-screen bg-gradient-to-br from-[#FDF7EE] via-[#f8f4ff] to-[#FDF7EE] pt-20 sm:pt-24 px-3 sm:px-4 lg:px-8 pb-8 sm:pb-12">
        <div className="max-w-7xl mx-auto">
          {/* Header Section */}
          <div className="mb-6 sm:mb-8">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#3E1C96] to-[#F04438] mb-2 sm:mb-3">
              📜 History Pelaporan
            </h1>
            <p className="text-gray-600 text-base sm:text-lg">Riwayat semua laporan yang pernah kamu buat</p>
          </div>

          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 sm:py-32">
              <Loader2 className="w-10 h-10 sm:w-12 sm:h-12 animate-spin text-[#3E1C96] mb-4" />
              <p className="text-gray-500 text-sm">Memuat data...</p>
            </div>
          ) : laporan.length === 0 ? (
            <div className="bg-white/80 backdrop-blur-sm p-8 sm:p-12 rounded-2xl shadow-xl text-center border border-gray-100">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-gray-800 mb-2">Belum Ada Laporan</h3>
              <p className="text-gray-600 text-sm sm:text-base">Kamu belum pernah membuat laporan. Yuk mulai buat laporan pertamamu!</p>
            </div>
          ) : (
            <>
              {/* Stats Summary */}
              <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-6">
                <div className="bg-white/80 backdrop-blur-sm rounded-lg sm:rounded-xl p-3 sm:p-4 border border-purple-100 shadow-sm">
                  <p className="text-xs sm:text-sm text-gray-600 mb-1">Total Laporan</p>
                  <p className="text-xl sm:text-2xl font-bold text-[#3E1C96]">{laporan.length}</p>
                </div>
                <div className="bg-white/80 backdrop-blur-sm rounded-lg sm:rounded-xl p-3 sm:p-4 border border-green-100 shadow-sm">
                  <p className="text-xs sm:text-sm text-gray-600 mb-1">Selesai</p>
                  <p className="text-xl sm:text-2xl font-bold text-green-600">
                    {laporan.filter(l => l.status === 'selesai').length}
                  </p>
                </div>
                <div className="bg-white/80 backdrop-blur-sm rounded-lg sm:rounded-xl p-3 sm:p-4 border border-yellow-100 shadow-sm">
              ...
              <div className="hidden lg:block bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-gray-200">
              ...
              <div className="lg:hidden space-y-3 sm:space-y-4">
                {laporan.map((item) => (
                  <div 
                    key={item.id}
                    className="bg-white/90 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-xl transition-shadow duration-200"
                  >
                    ...
                    <div className="relative">
                      {item.laporan_foto ? (
                        ...
                        <div className="w-full h-48 sm:h-56 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center text-gray-400 font-medium">
                          No Photo Available
                        </div>
                      )}
                      ...
                      <div className="absolute top-3 right-3">
                        <span
                          className={`inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full font-semibold shadow-lg backdrop-blur-sm ${
                            item.status === 'selesai'
                    ...
                    <div className="p-4 sm:p-5">
                      {/* Title */}
                      <h3 className="text-lg sm:text-xl font-bold text-[#3E1C96] mb-3 leading-tight">
                        {item.judul}
                      </h3>
                      ...
                      <div className="space-y-2.5">
                        {/* Location */}
                        <div className="flex items-start gap-2.5 p-2.5 bg-red-50/50 rounded-lg">
                        ...
                        {/* Date & Time */}
                        <div className="flex items-start gap-2.5 p-2.5 bg-blue-50/50 rounded-lg">
                          <Calendar className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 flex-shrink-0 mt-0.5" />
                          <div className="flex-1">
                            <p className="text-xs text-gray-500 mb-0.5">Tanggal Laporan</p>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">
                              Pukul {new Date(item.created_at).toLocaleTimeString('id-ID', {
                                hour: '2-digit',
                                minute: '2-digit'
                              })} WIB
                            </p>
        ...
        >
          <div className="relative max-w-4xl w-full max-h-[90vh]">
            <img 
              src={selectedImage} 
              alt="Preview" 
              className="w-full h-auto max-h-[90vh] object-contain rounded-xl sm:rounded-2xl shadow-2xl"
            />
            <button 
              onClick={() => setSelectedImage(null)}
              className="absolute -top-2 -right-2 sm:-top-4 sm:-right-4 bg-white text-gray-800 rounded-full w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center shadow-lg hover:bg-gray-100 transition font-bold text-lg"
            >
              ✕
            </button>
          </div>
